...
Download the Capture Growth Charts plugin available in the [DHIS2 App Hub](https://apps.dhis2.org/app/09f48f78-b67c-4efa-90ad-9ac2fed53bb8) in the **App Management** application on DHIS2.
...
If one of the data elements is missing, growth charts using that data element will not be displayed.
...
Select the stage where growth variables currently are or will be stored.
...
3. Add the custom references you want to use.
   The structure of the custom references has to be the same as the one in the example above,
   but the contents of **percentileDatasetValues** and **zScoreDatasetValues** should be changed to your custom references.
   However, you don't need to add references for both percentiles and z-scores, you can choose to only add one of them if desired.
4. Add other indicators if you want, but make sure that the key maps to the right category and gender.
   The indicator key should be one of the following:
...
Now you can set `customReferences` to `true` in the config.
This will make the plugin use the custom references you have created.
If you want to use the default references, you can set `customReferences` to `false` in the config.
This will make the plugin use the WHO references.
Make sure to also alter the `usePercentiles` in the chart config to match the references you are using, z-scores or percentiles. 
